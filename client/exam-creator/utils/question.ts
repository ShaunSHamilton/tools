...
export function change_question(
  ...
  setQuestionSets(
    questionSets.map((qt) => ({
      ...qt,
      questions: qt.questions.map((q: any) =>
...
export function remove_question(
  ...
  const question_type = questionSets.find((qt) =>
    qt.questions.some((q: any) => q.id === question.id)
  );
  ...
  setQuestionSets(
    questionSets.map((qt) => ({
      ...qt,
      questions: qt.questions.filter((q: any) => q.id !== question.id),
    }))
  );
}
...
export function calculateGenerationMetrics(
  ...
  const questionIds = questions.map((qs: any) => qs.map((q: any) => q.id));
  ...
  const answers = nonDeprecatedGenerations.map((gen) =>
    gen.questionSets.flatMap((qs: any) => qs.questions).flatMap((q: any) => q.answers)
  );
...
export function getAnswerStatus(
  ...
  const stagingCount =
    stagingExams?.filter?.((exam) =>
      exam.questionSets.some((qs: any) =>
        qs.questions.some((q: any) => q.answers.some((a: any) => a === answerId))
      )
    )?.length ?? 0;
...
export function getQuestionStatus(
  ...
  const stagingCount =
    stagingExams?.filter?.((exam) =>
      exam.questionSets.some((qs: any) =>
        qs.questions.some((q: any) => q.id === questionId)
      )
    )?.length ?? 0;
...
export function getQuestionSetStatus(
  ...
  const stagingCount =
    stagingExams?.filter?.((exam) =>
      exam.questionSets.some((qs: any) => qs.id === questionSetId)
    )?.length ?? 0;
