const API_PREFIX = import.meta.env.VITE_API_PREFIX ?? "/exam-creator";
...
export async function discardExamStateById(
  ...
  const res = await authorizedFetch(`/api/state/exams/${examId}`, {
    method: "PUT",
  });
...
export async function getModerations({
  ...
  const url = new URL(`${API_PREFIX}/api/attempts`, window.location.href);
  if (status) {
...
export async function getStatusPing() {
  ...
  const res = await fetch(`${API_PREFIX}/status/ping`, {
    method: "GET",
    headers: {
